...
exports.detailPage = async (req, res) => {
  try {
    const data = await service.getDetail(req.params.registerno);
    ...
    res.status(err.status || 500).send(err.message || 'Gagal mengambil detail.');
  }
};
...
exports.detail = async (req, res) => {
  try {
    const data = await service.getDetail(req.params.registerno);
    if (!data)
      return res.status(404).json({ success: false, msg: 'Register tidak ditemukan.' });
    res.json({ success: true, data });
  } catch (err) {
    sendError(res, err);
  }
...
exports.remove = async (req, res) => {
  try {
    await service.remove(req.params.registerno);
...
exports.print = async (req, res) => {
  ...
  try {
    ...
    const db = require('../db');
    const paramRows = await db.query('SELECT company FROM public.param LIMIT 1');
